...
echo "    Locktime: $LOCKTIME (refund after this block)"
...
# Execute the Python script (zkcp_complete_tx.py) that creates the proper scriptSig
python3 zkcp_complete_tx.py "$REAL_K" "$LOCKTIME" "$REDEEM_SCRIPT" "$TXID" "$VOUT" $AMOUNT
